...
export class OrderService {
  ...
  async placeOrder(dto: OrderDto, userId: number) {
    ...
    await this.prisma.order.update({where: {id: order.id}, data: {order_payment_id: payment.id}})
    return payment
  }
  ...
  async updateStatus(dto: PaymentStatusDto) {
    ...
    if(dto.event === 'payment.waiting_for_capture') return await this.eventWaitingForCapture(dto.object.id)
    if(dto.event === 'payment.succeeded'){
      const orderId = Number(dto.object.description.split ('#')[1]);
      return await this.eventPaymentSucceeded(orderId)
    }
    if(dto.event === 'refund.succeeded') return this.eventRefundSucceeded(dto.object.payment_id)
  ...
  async refund(dto: RefundDto) {
    ...
    const payment = await this.paymentService.createRefund(order.order_payment_id, order.totalPrice)
    return payment
  }
